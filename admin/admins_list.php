  ...
  <div class="content-wrapper">
    ...
    <?php 
      ...
      if (empty($_POST['search'])) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM admin ORDER BY id DESC LIMIT $offset,$numOfrecs");
        $stmt->execute();
        ...
        $stmt = $pdo->prepare("SELECT * FROM admin WHERE title LIKE '%$searchKey%' ORDER BY id DESC");
        $stmt->execute();
        $rawresult = $stmt->fetchAll();
        $total_pages = ceil(count($rawresult)/ $numOfrecs);

        $stmt = $pdo->prepare("SELECT * FROM admin WHERE title LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$numOfrecs");
        $stmt->execute();
        $result = $stmt->fetchAll();
    ...
    <section class="content">
      <div class="container">
        ...
        <a href="admin_add.php" class="btn btn-success" style="float: left; margin: 10px;">Create New New</a>
        <div class="table">
          <table class="table table-bordered">
            ...
            foreach ($result as $value) { ?>
              <tr>
              <td><?php echo $i; ?></td>
              <td><?php echo escape($value['username']); ?></td>
              <td><?php echo escape(date("y-M-d h:i", strtotime($value['created_at'])))." "; ?><i class="far fa-clock"></i>hours ago</td>


              <td>
                <a href="admin_edit.php?id=<?php echo $value['id']; ?>" class="btn btn-primary">Edit</a>
                <a href="admin_delete.php?id=<?php echo $value['id']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure want to delete this item')">Delete</a>
              </td>
            </tr>
            <?php 
            $i++;
