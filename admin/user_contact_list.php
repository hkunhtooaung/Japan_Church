  ...
  <div class="content-wrapper">
    ...
    <?php 
      ...
      if (empty($_POST['search'])) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM user_contact ORDER BY id DESC");
        $stmt->execute();
        $rawresult = $stmt->fetchAll();
        $total_pages = ceil(count($rawresult)/ $numOfrecs);

        $stmt = $pdo->prepare("SELECT * FROM user_contact ORDER BY id DESC LIMIT $offset,$numOfrecs");
        $stmt->execute();
        ...
        $stmt = $pdo->prepare("SELECT * FROM user_contact WHERE title LIKE '%$searchKey%' ORDER BY id DESC");
        $stmt->execute();
        $rawresult = $stmt->fetchAll();
        $total_pages = ceil(count($rawresult)/ $numOfrecs);

        $stmt = $pdo->prepare("SELECT * FROM user_contact WHERE title LIKE '%$searchKey%' ORDER BY id DESC LIMIT $offset,$numOfrecs");
        $stmt->execute();
        $result = $stmt->fetchAll();
        ...
        <div class="table">
          <table class="table table-bordered">
            ...
            foreach ($result as $value) { ?>
              ...
              <td><?php echo escape($value['subject']); ?></td>
              <td><?php echo escape(date("y-M-d h:i", strtotime($value['created_at'])))." "; ?><i class="far fa-clock"></i>hours ago</td>


              <td>
                <a href="user_contact_view.php?id=<?php echo $value['id']; ?>" class="btn btn-primary">View Detail</a>
                <a href="user_contact_delete.php?id=<?php echo $value['id']; ?>" class="btn btn-danger" onclick="return confirm('Are you sure want to delete this item')">Delete</a>
              </td>
            </tr>
            <?php 
            $i++;
