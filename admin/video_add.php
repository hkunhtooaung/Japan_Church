...
if ($_POST) {
if (empty($_POST['title']) || empty($_POST['description']) || empty($_FILES['image']['name']) || empty($_POST['vlink'])) {
    ...
    if (empty($_POST['video']['name'])) {
      $vlinkError = 'Video link is required';
    }
} else {
	...
	$fileType = pathinfo($file, PATHINFO_EXTENSION);
		...
		$stmt = $pdo->prepare('INSERT INTO videos (title,description,image,video_link) VALUES (:title,:description,:image,:vlink)');
		$result = $stmt->execute(
			[':title'=>$title, ':description'=>$description, ':image'=>$image, ':vlink'=>$vlink]
		);

		if ($result) {
			echo "<script>alert('Successfully Created');location.href='videos_list.php';</script>";
		}
	}
}
}
		...
		<h1>CREATE NEW</h1>
			<form action="" method="POST" class="" enctype="multipart/form-data">
				...
				<div class="form-group">
	             ...
	             <div class="form-group">
	               <label for="">Video Link</label><p style="color: red;"><?php echo empty($vlinkError) ? '' : '*'.$vlinkError; ?></p>
	               <input type="text" name="vlink" value="" class="form-control">
	             </div>
				<div class="form-outline mb-4">
